...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!accessToken) {
      return NextResponse.json({ skipped: true, reason: 'No GA4 token' })
    }
    ...
    if (!ga4Res.ok) return NextResponse.json({ skipped: true, reason: 'GA4 API error' })
    ...
    await notifySlack(`📊 [AskHistory] 월간 AdSense 최적화 리포트\n\n📈 광고 추가 추천:\n${topLines || '없음'}\n\n📉 CTA 전환 권장 (이탈률 80%+):\n${bounceLines || '없음'}`)
